...
namespace sio
{
    ...
    public:
        packet(string const& nsp,message::ptr const& msg,int pack_id = -1,bool isAck = false) :
        ...
        packet(type type,string const& nsp= string(),message::ptr const& msg = message::ptr()) :
            _frame(frame_message),
            _type(type),
            _nsp(nsp),
            _pack_id(-1),
        ...
        //return true if need to parse buffer.
        bool parse(string const& payload_ptr) {
        ...
        bool accept(string& payload_ptr, vector<shared_ptr<const string> >&buffers) {
